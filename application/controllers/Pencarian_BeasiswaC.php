...
{	
	...
	public function scrapping_detail_beasiswa_by_universitas(){
		...
		$html_load = file_get_html('http://beasiswa.id/2017/beasiswa-s2-di-ugm-universitas-gadjah-mada/');
		$i = 0;
		if($html_load && is_object($html_load) && isset($html_load->nodes)){
			foreach ($html_load->find('div#ht-content .ht-container') as $data ) {
				...
				$sub = array();
					...
					foreach ($a->find('div.entry-content') as $b) {

						foreach ($b->find('ol li') as $c) {
							$result['first']= $c->plaintext;
				


					// $result['second']= $b->children(5)->plaintext;
					// $result['third']= $b->children(6)->plaintext;
